...
const (
	infuraURL       = "https://mainnet.infura.io/v3/d339bf98ee6b40f291c973b2cf924feb"
...
func main() {
	...
	wethAddresHex := common.HexToAddress(wethAddress)

	wethBalance, err := getERC20Balance(client, wethAddresHex, address)
	if err != nil {
		log.Fatalf("Failed to get WETH balance: %v", err)
	}

	ethInUSD := new(big.Float).SetInt(balance)
	ethInUSD.Mul(ethInUSD, big.NewFloat(ethPrice))

	wethInUSD := new(big.Float).SetInt(wethBalance)
	wethInUSD.Mul(wethInUSD, big.NewFloat(ethPrice))

	fmt.Printf("ETH Balance: %s\n", balance.String())
	fmt.Printf("ETH in USD: $%.2f\n", ethInUSD)
	fmt.Printf("WETH Balance: %s\n", wethBalance.String())
	fmt.Printf("WETH in USD: $%.2f\n", wethInUSD)
}

func getPriceInUSD(client *ethclient.Client, priceFeedAddress string) (float64, error) {
	...
	priceUSD, _ := priceFloat.Quo(priceFloat, big.NewFloat(1e18)).Float64()
